...
void map_page_entry(uint64_t *pml, uint64_t va, uint64_t size, uint64_t pa, uint64_t perm)
{
	uint64_t *pte_ptr;
	int i;
	for (i = 0; i < size/PG_SIZE; i += PG_SIZE) {
...
void page_table_init()
{
	...
	map_page_entry(pml, KERNBASE + 0xb800, 4 * 4096, (uint64_t)phys_base, 0);
}
